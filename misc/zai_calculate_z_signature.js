...
function calcSignature(prompt, userId) {
    const SECRET = "key-@@@@)))()((9))-xxxx&&&%%%%%";
    const ts = String(Date.now());
    const rid = crypto.randomUUID();

    const sp = `requestId,${rid},timestamp,${ts},user_id,${userId}`;

    const bytes = new TextEncoder().encode(prompt);
    let bs = "";
    for (let i = 0; i < bytes.length; i += 32768)
        bs += String.fromCharCode(...Array.from(bytes.slice(i, i + 32768)));
    const b64 = btoa(bs);

    const msg = sp + "|" + b64 + "|" + ts;
    const tw = String(Math.floor(ts / 300000));

    const dk = hmacSha256Pure(SECRET, tw);
    const sig = hmacSha256Pure(dk, msg);

    return { signature: sig, timestamp: ts, requestId: rid, sortedPayload: sp };
}